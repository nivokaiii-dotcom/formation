...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_user'])) {
    ...
    if ($userId === (int)$_SESSION['user']['id']) {
        $message = "<div class='alert alert-warning'>Action impossible : Vous ne pouvez pas vous supprimer vous-même.</div>";
    } else {
        try {
...
<style>
    ...
    /* Ces styles s'assurent que la page prend toute la hauteur */
    html, body {
        height: 100%;
        margin: 0;
    }
    ...
    .main-content {
        flex: 1 0 auto; /* Cette section va s'étendre pour pousser le footer */
    }
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <div class="fw-bold text-title"><?= htmlspecialchars($u['username']) ?></div>
                                        <div class="text-muted small">ID: <?= $u['discord_id'] ?></div>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <form method="POST" class="d-flex align-items-center gap-2">
                                    ...
                                    <select name="role" class="form-select form-select-sm fw-bold" style="width: 110px;">
                                        <option value="user" <?= $u['role'] === 'user' ? 'selected' : '' ?>>User</option>
                                        <option value="admin" <?= $u['role'] === 'admin' ? 'selected' : '' ?>>Admin</option>
                                    </select>
                                    <button type="submit" name="update_role" class="btn btn-primary btn-sm rounded-pill">
                            ...
                            <td class="text-end pe-4">
                                <form method="POST" onsubmit="return confirm('Confirmer la suppression ?');">
                                    <input type="hidden" name="user_id" value="<?= $u['id'] ?>">
                                    <button type="submit" name="delete_user" class="btn btn-sm btn-delete rounded-pill px-3 fw-bold">
